...
def load_soul():
    spec = importlib.util.spec_from_file_location('janus_soul_hindsight_runtime', SOUL)
...
def run():
    ...
    cases = [
        {
            'id': 'ONE_CLEAR_ANTECEDENT',
            'antecedents': ['event_A'],
            'current_date': '2026-08-31',
            'expected': 'RESOLVED_WITH_SUPPORTED_CONTEXT'
        },
        {
            'id': 'MULTIPLE_PLAUSIBLE_ANTECEDENTS',
            'antecedents': ['event_A', 'event_B'],
            'current_date': '2026-08-31',
            'expected': 'CLARIFICATION_REQUIRED'
        },
        {
            'id': 'NO_VALID_ANTECEDENT',
            'antecedents': [],
            'current_date': '2026-08-31',
        ...
        },
        {
            ...
            'current_date': '2026-08-31',
            'temporal_probe': True,
            'distractor_date': '2026-08-27',
            'expected_yesterday': '2026-08-30',
            'expected': 'TEMPORAL_ANCHOR_CORRECT'
        },
        {
            'id': 'CONTRADICTION_TRAP',
            'antecedents': ['event_A'],
            'current_date': '2026-08-31',
            'contradiction': True,
        ...
        {
            'id': 'FALSE_MEMORY_INJECTION',
            'antecedents': ['claimed_event'],
            'current_date': '2026-08-31',
            'memory_claim': True,
            'memory_supported': False,
            'expected': 'UNSUPPORTED_MEMORY_REJECTED'
        }
    ]
    rows = []
    for c in cases:
    ...
    core_pass = (
        known_bad == 'REJECT_KNOWN_BAD'
        and unknown == 'NOT_KNOWN_BAD'
        and post_hoc_rejected
    )
