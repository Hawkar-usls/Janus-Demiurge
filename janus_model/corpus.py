...
def build_corpus(registry:Path,out_dir:Path,max_train_bytes=2_000_000,max_holdout_bytes=300_000):
    ...
    source_digest=hashlib.sha256(); source_file_count=0; source_total_bytes=0
    ...
    for path in iter_source_files(registry):
        ...
        # Every eligible source-bearing file participates in this digest, even
        # when the train/holdout byte caps are already full.
        source_digest.update(f'{rel}\0{file_sha}\0{len(raw)}\n'.encode())
        source_file_count += 1
...
def main():
    ...
    print(json.dumps({k:m[k] for k in ('source_commit','source_digest','source_file_count','source_total_bytes','selected_record_count','train_bytes','holdout_bytes')},indent=2))
