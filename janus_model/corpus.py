...
def _load_frontier_memory(training_path:Path,manifest_path:Path):
    pack=training_path.read_bytes()
    manifest=json.loads(manifest_path.read_text(encoding='utf-8'))
    if manifest.get('schema')!=FRONTIER_MEMORY_SCHEMA:
        raise RuntimeError('FRONTIER_MEMORY_MANIFEST_SCHEMA_REJECTED')
    if manifest.get('status')!='READY_READ_ONLY_TRAINING_MEMORY':
        raise RuntimeError('FRONTIER_MEMORY_STATUS_REJECTED')
    if manifest.get('source_repository')!='Hawkar-usls/Janus-Fundamentum':
        raise RuntimeError('FRONTIER_MEMORY_SOURCE_REPOSITORY_REJECTED')
    if manifest.get('P_VS_NP')!='OPEN':
        raise RuntimeError('FRONTIER_MEMORY_P_VS_NP_MUST_REMAIN_OPEN')
    if manifest.get('training_only') is not True:
        raise RuntimeError('FRONTIER_MEMORY_TRAIN_ONLY_REQUIRED')
    if manifest.get('adaptive_holdout_inclusion') is not False or manifest.get('frozen_anchor_inclusion') is not False:
        raise RuntimeError('FRONTIER_MEMORY_EVALUATION_LEAKAGE_REJECTED')
    if manifest.get('training_material_is_truth') is not False or manifest.get('contribution_grants_authority') is not False:
        raise RuntimeError('FRONTIER_MEMORY_EPISTEMIC_FIREWALL_REJECTED')
    if manifest.get('source_execution') is not False or manifest.get('cross_repository_write') is not False:
        raise RuntimeError('FRONTIER_MEMORY_EXECUTION_AUTHORITY_REJECTED')
    authority=manifest.get('authority') or {}
    if authority.get('read_only_source') is not True or authority.get('authority_delta')!=0:
        raise RuntimeError('FRONTIER_MEMORY_AUTHORITY_REJECTED')
    for key in ('may_mutate_source_repository','may_change_active_lineage','may_change_proof_ladder','may_promote_theorem','may_promote_runtime'):
        if authority.get(key) is not False:
            raise RuntimeError(f'FRONTIER_MEMORY_AUTHORITY_REJECTED:{key}')
    if manifest.get('training_pack_sha256')!=sha256_bytes(pack):
        raise RuntimeError('FRONTIER_MEMORY_TRAINING_PACK_HASH_MISMATCH')
    if manifest.get('training_bytes')!=len(pack) or len(pack)<=0:
...
def build_corpus(registry:Path,out_dir:Path,max_train_bytes=2_000_000,max_holdout_bytes=300_000,keymaster_training_path:Path|None=None,keymaster_manifest_path:Path|None=None,keymaster_config_path:Path|None=None,frontier_training_path:Path|None=None,frontier_manifest_path:Path|None=None):
    ...
    if (frontier_training_path is None)!=(frontier_manifest_path is None):
        raise RuntimeError('FRONTIER_MEMORY_TRAINING_AND_MANIFEST_MUST_BE_PAIRED')
    if frontier_training_path is None:
        default_train_exists=DEFAULT_FRONTIER_MEMORY_TRAINING.is_file()
        default_manifest_exists=DEFAULT_FRONTIER_MEMORY_MANIFEST.is_file()
        if default_train_exists != default_manifest_exists:
            raise RuntimeError('FRONTIER_MEMORY_DEFAULT_PAIR_INCOMPLETE')
        if default_train_exists:
            frontier_training_path=DEFAULT_FRONTIER_MEMORY_TRAINING
            frontier_manifest_path=DEFAULT_FRONTIER_MEMORY_MANIFEST

    frontier_text=''
    frontier_manifest=None
    if frontier_training_path is not None:
    ...
    frontier_summary={
        'enabled':frontier_manifest is not None,
        'schema':frontier_manifest.get('schema') if frontier_manifest else None,
        'source_repository':frontier_manifest.get('source_repository') if frontier_manifest else None,
        'source_commit':frontier_manifest.get('source_commit') if frontier_manifest else None,
        'selected_ref':frontier_manifest.get('selected_ref') if frontier_manifest else None,
        'intake_sha256':frontier_manifest.get('intake_sha256') if frontier_manifest else None,
        'training_pack_sha256':frontier_manifest.get('training_pack_sha256') if frontier_manifest else None,
        'training_bytes':len(frontier_text.encode('utf-8')) if frontier_manifest else 0,
        'included_file_count':frontier_manifest.get('included_file_count',0) if frontier_manifest else 0,
        'training_only':True if frontier_manifest else None,
        'adaptive_holdout_inclusion':False if frontier_manifest else None,
        'frozen_anchor_inclusion':False if frontier_manifest else None,
