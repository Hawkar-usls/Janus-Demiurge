...
class CausalAckLedgerTests(unittest.TestCase):
    ...
    def test_exact_chain_is_complete_trace_but_not_scientific_causation(self):
        ...
        self.assertTrue(ack["exact_workflow_trigger_claimed"])
        self.assertFalse(ack["scientific_causation_claimed"])
        self.assertEqual(ack["candidate_routes"][0]["origin_message_id"], "M1")
        self.assertEqual(bundle["stats"]["exact_interaction_birth_count"], 1)

    def test_duplicate_origin_routes_stay_ambiguous(self):
        result, peers, inboxes = self.fixture()
        ...
        self.assertEqual(bundle["stats"]["status_counts"]["AMBIGUOUS_MULTI_ROUTE_TRACE"], 1)
        self.assertFalse(bundle["causal_acks"][0]["exact_workflow_trigger_claimed"])

    def test_failed_fetch_cannot_form_exact_trace(self):
        result, peers, inboxes = self.fixture()
    ...
    def test_direct_source_pointer_can_form_exact_trace(self):
        result, peers, inboxes = self.fixture()
