...
from janus_model.keymaster import eligible_tracked_files


ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "janus_model/keymaster/PRIMARY_REPOSITORY_CONTRIBUTORS-v1.json"
EXPECTED = {
    "Hawkar-usls/TOPA",
...
class JanusKeymasterTests(unittest.TestCase):
    def test_exact_five_primary_contributors_and_authority_firewalls(self):
        ...
        self.assertEqual({row["repository"] for row in obj["contributors"]}, EXPECTED)
        self.assertEqual(len({row["id"] for row in obj["contributors"]}), 5)
    ...
    def test_keymaster_pack_requires_five_nonzero_and_never_enters_eval(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(loaded["contributor_count"], 5)
            bad = dict(manifest)
